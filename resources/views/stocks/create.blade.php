                    ...
                    <div class="col-md-5 col-lg-4">
                        <form action="/stocks/store" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="name" required placeholder="Введите название склада">
                            </div>
                            <div class="form-group">
                                <label for="department_id">Филиал: *</label>
                                <select name="department_id" class="form-control" id="department_id" required>
                                    @foreach($departments as $department)
                                    <option {{ $department->id_department == $selectedDepartment ? 'selected' : '' }} value="{{ $department->id_department }}">{{ $department->v_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="location_id">Участок: *</label>
                                <select name="location_id" class="form-control" id="location_id" required></select>
                            </div>
                            <div class="form-group">
                                <label for="users">Ответственные: *</label>
                                <select name="users[]" class="form-control selectpicker" required data-style="btn-secondary" data-live-search="true" multiple id="users"></select>
                            </div>
                            <div class="form-group">
